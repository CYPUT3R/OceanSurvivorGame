...
var _load_screen_path : String = "res://Scenes/load.tscn"
var _load_screen = load(_load_screen_path)
var _loaded_resouce: PackedScene
...
func  _process(delta):
	...
	match load_status:
		...
		3: #? THREAD_LOAD_LOADED
			_loaded_resouce = ResourceLoader.load_threaded_get(_scene_path)
			emit_signal("progress_changed", 1.0)
			emit_signal("load_done")
			get_tree().change_scene_to_packed(_loaded_resouce)
